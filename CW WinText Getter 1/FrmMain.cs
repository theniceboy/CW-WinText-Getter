    ...
    public partial class FrmMain : DevComponents.DotNetBar.Metro.MetroForm
    {
        ...
        private void kh_OnKeyDownEvent(object sender, KeyEventArgs e)
        {
            if (e.KeyData == (Keys.Z | Keys.Alt))
            {
                try
                {
                    Clipboard.SetDataObject(textv.Text, true);
                }
                catch { }
            }
        }
        ...
        private void nowstate_Click(object sender, EventArgs e)
        {
            if (state == 1)
            {
                state = 2;
                nowstate.TextColor = Color.DarkRed;
                nowstate.Text = "Pausing";
                timer.Enabled = false;
            }
            else
            {
        ...
        private void TopMost_Click(object sender, EventArgs e)
        {
            topmost = 3 - topmost;
            TopMost = !TopMost;
            dotopmost.Checked = !dotopmost.Checked;
        }
    }
